...
const AI = {
    ...
    shouldContinue(game) {
        ...
        // Hot Dice! Always continue if 0 dice are left
        if (diceRemaining === 0) return true;

        // Force AI to keep rolling if they haven't gotten on the board and are under 500
        if (!player.onBoard && turnTotal < 500) {
            return true;
        }
    ...
    getThreshold(diceCount) {
        // Minimum score threshold to stop rolling, mapping how risky rolling N dice is.
        const thresholds = {
            1: 450,  // Rolling 1 die risks Farkle 66% of the time, so only do it if turn score < 450
            2: 350,  // Rolling 2 dice risks Farkle 44% of the time
            3: 200,  // Rolling 3 dice is safer
            4: 150,
            5: 100,
            6: 0     
        };
