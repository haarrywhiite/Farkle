    ...
    saveState() {
        if (this.gameState !== 'GAME_OVER') {
            const state = {
                ...
                currentRollScore: this.currentRollScore,
                gameState: this.gameState,
                tournament: this.tournament
            };
            try { localStorage.setItem('farkleGameState', JSON.stringify(state)); } catch (e) {}
        }
    }

    clearState() {
        try { localStorage.removeItem('farkleGameState'); } catch (e) {}
    }

    loadState() {
        try {
            const saved = localStorage.getItem('farkleGameState');
            if (saved) {
                ...
                this.turnTotal = state.turnTotal;
                this.currentRollScore = state.currentRollScore;
